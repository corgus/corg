...
class FGMembersite
{
    ...
    function CheckLoginInDB2($username,$password)
    {
        if(!$this->DBLogin())
        {
            $this->HandleError("Database login failed!");
        ...
        $username = $this->SanitizeForSQL($username);
        $pwdmd5 = md5($password);
        $qry = "Select id_user, name, email from $this->tablename where and username='$username' and password='$pwdmd5' and confirmcode='y'";
